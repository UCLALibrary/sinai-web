...
/**
 * A verticle that periodically queries the KatIkon PostgreSQL database and updates our Solr index of the metadata.
 */
public class MetadataHarvestVerticle extends AbstractSinaiVerticle {
    ...
    static class MetadataHarvestHandler implements Handler<Long> {
        ...
        /**
         * Data structure containing information for mapping database columns to Solr fields. - name: name of the
         * destination Solr field - alias: qualified name (
         * <table>
         * .<column>) of the source database column (empty if "name" can identify the column) - type: type of the
         * destination Solr field (must be either "string" or "int") - multiValued: whether field is multi-valued or not
         */
        class MetadataHarvestDBFields {
            ...
            public MetadataHarvestDBFields(final String aName, final String aAlias, final String aType,
        ...
        private final SolrClient mySolrServer;
        ...
        public MetadataHarvestHandler(final Configuration aConfig) {
            ...
            myDatabaseUrl = "jdbc:postgresql://" + databaseProperties.getString("host") + ":" +
                    databaseProperties.getString("port") + "/" + databaseProperties.getString("database");
        ...
        private String solrDynamicFieldSuffix(final String type, final Boolean multiValued) {
            String ret = "";
            if (type.equals("int")) {
                ret = "_i";
         ...
         */
        private void updateSolr(final String doctype, final MetadataHarvestDBFields[] fields, final String sql,
                final Connection conn, final String multiValuedFieldDelimiter) {
                ...
                while (rs.next()) {
                    ...
                    for (int i = 0; i < fields.length; i++) {
                        ...
                        if (fields[i].name.equals("id")) {
                            ...
                            solrFieldName =
                                    fields[i].name + solrDynamicFieldSuffix(fields[i].type, fields[i].multiValued);
        ...
        public void handle(final Long arg0) {
            ...
            try (final Connection conn = DriverManager.getConnection(myDatabaseUrl, myDatabaseProps)) {
                ...
                final MetadataHarvestDBFields[] manuscriptsFields = {
                    ...
                    new MetadataHarvestDBFields("secondary_languages",
                            "array_remove(array_replace(ARRAY[m.secondary_language_1, m.secondary_language_2, m.secondary_language_3], '', NULL), NULL)",
                            "string", true),
                    ...
                    new MetadataHarvestDBFields("previous_catalog_information", "m.previous_catalog_information",
                            "string", false) };
                final String manuscriptsSql = "SELECT " + String.join(",", Arrays.stream(manuscriptsFields).map(s -> {
                    return s.alias + " AS " + s.name;
                }).toArray(String[]::new)) +
                        " FROM manuscripts AS m INNER JOIN manuscript_types AS mt ON m.manuscript_type_id = mt.id";
                ...
                final MetadataHarvestDBFields[] utoFields = {
                    ...
                    new MetadataHarvestDBFields("script_characterization", "uto.script_characterization", "string",
                            false),
                    new MetadataHarvestDBFields("secondary_languages",
                            "array_remove(array_replace(ARRAY[uto.secondary_language_1, uto.secondary_language_2, uto.secondary_language_3], '', NULL), NULL)",
                            "string", true),
                    ...
                    new MetadataHarvestDBFields("related_undertext_objects", "uto.related_undertext_objects", "string",
                            false),
                ...
                final MetadataHarvestDBFields[] folioFields = {
                    ...
                    new MetadataHarvestDBFields("leading_conjoin_component_type",
                            "leading_conjoins.leading_conjoin_component_type", "string", false),
                    new MetadataHarvestDBFields("leading_conjoin_folio_number",
                            "leading_conjoins.leading_conjoin_folio_number", "string", false),
                    new MetadataHarvestDBFields("leading_conjoin_folio_side",
                            "leading_conjoins.leading_conjoin_folio_side", "string", false),

                    new MetadataHarvestDBFields("trailing_conjoin_component_type",
                            "trailing_conjoins.trailing_conjoin_component_type", "string", false),
                    new MetadataHarvestDBFields("trailing_conjoin_folio_number",
                            "trailing_conjoins.trailing_conjoin_folio_number", "string", false),
                    new MetadataHarvestDBFields("trailing_conjoin_folio_side",
                            "trailing_conjoins.trailing_conjoin_folio_side", "string", false),
                ...
                final MetadataHarvestDBFields[] underTextLayerFields = {
                    ...
                    new MetadataHarvestDBFields("secondary_languages",
                            "array_remove(array_replace(ARRAY[tl.secondary_language_1, tl.secondary_language_2, tl.secondary_language_3], '', NULL), NULL)",
                            "string", true),
                    ...
                    new MetadataHarvestDBFields("nontextual_content_present", "tl.nontextual_content_present",
                            "boolean", false),
                ...
                final String underTextLayerSql =
                        "SELECT " + String.join(",", Arrays.stream(underTextLayerFields).map(s -> {
                            return s.alias + " AS " + s.name;
                        }).toArray(String[]::new)) +
                                " FROM text_layers AS tl INNER JOIN manuscript_components AS mc ON tl.manuscript_component_id = mc.id WHERE tl.type = 'UnderTextLayer' AND tl.undertext_object_id IS NOT NULL";
                ...
                final MetadataHarvestDBFields[] overTextLayerFields = {
                    ...
                    new MetadataHarvestDBFields("notes", "tl.notes", "string", false) };
                final String overTextLayerSql =
                        "SELECT " + String.join(",", Arrays.stream(overTextLayerFields).map(s -> {
                            return s.alias + " AS " + s.name;
                        }).toArray(String[]::new)) +
                                " FROM text_layers AS tl INNER JOIN manuscript_components AS mc ON tl.manuscript_component_id = mc.id WHERE tl.type = 'OverTextLayer'";
